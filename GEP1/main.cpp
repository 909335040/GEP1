...
void loadFromFile() {
	...
	if (!keyData.is_open()) {
		...
		while (!typeData.eof()) {
			...
			modelData >> modelTransforms.x>>modelTransforms.z>>modelTransforms.rot;
			window.writeModelData(modelTransforms);
...
int main(int argc, char** argv)
{
	...
	thread	t2(moveModel);
	thread	t1(winsockStartupAndRun);  
	loadFromFile();
